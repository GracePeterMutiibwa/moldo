        ...
        while i < len(inner):
            if inner[i] == '{':
                ...
                if j == -1:
                    ...
                    out += str(ns.get(inner[i+1:j], '')); i = j + 1
            else:
                out += inner[i]; i += 1
        return out
...
class MoldoInterpreter:
    ...
    async def _do_io(self, block: str, params: dict, ns: dict,
                     send: Send, recv: Recv) -> None:
        if block == 'print':
    ...
    async def _do_control(self, node: dict, block: str, params: dict,
                          ns: dict, send: Send, recv: Recv) -> None:
        if block == 'branch':
    ...
    def _do_math(self, block: str, params: dict, ns: dict) -> None:
        ...
        try:
            ...
            af, bf = 0.0, 0.0
        if   block == 'arithmetic': ns[r] = _BINARY_OPS.get(params.get('operator', '+'), operator.add)(af, bf)
        elif block == 'sqrt':       ns[r] = _math.sqrt(af)
        elif block == 'power':      ns[r] = af ** bf
        elif block == 'round':      ns[r] = round(af, int(bf))
        elif block == 'abs':        ns[r] = abs(af)
        elif block == 'floor':      ns[r] = _math.floor(af)
        elif block == 'ceil':       ns[r] = _math.ceil(af)
    ...
    def _do_text(self, block: str, params: dict, ns: dict) -> None:
        ...
        if   block == 'upper':    ns[r] = val.upper()
        elif block == 'lower':    ns[r] = val.lower()
        elif block == 'length':   ns[r] = len(val)
        elif block == 'join':     ns[r] = val + str(b)
        elif block == 'split':    ns[r] = val.split(str(b))
        elif block == 'contains': ns[r] = str(b) in val
        elif block == 'replace':
    ...
    async def _do_community(self, mold: str, block: str, params: dict,
                            ns: dict, send: Send) -> None:
